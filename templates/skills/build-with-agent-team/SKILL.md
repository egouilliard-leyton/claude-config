...
Before reading the plan, create a dedicated git branch for this build.

Extract the plan name from the path argument:
- Input: `.agents/plans/my-feature.plan.md`
- Branch: `plan/my-feature`

```bash
# Extract base name without extension
PLAN_NAME=$(basename "$ARGUMENTS[0]" .plan.md)
...
Report the active branch to the user before proceeding.
...
## Step 7: Post-Build Automation

Run these three sub-tasks in sequence after all agents complete and end-to-end validation passes.

### 7a — Update CLAUDE.md

Invoke the `claude-md-management:revise-claude-md` skill to reflect on what was built during the session and propose additions to CLAUDE.md / `.claude.local.md`:

```
Skill("claude-md-management:revise-claude-md")
```

Show the proposed diff to the user and apply with their approval (as the skill requires).

### 7b — Refresh codebase docs

Spawn 4 parallel `gsd-codebase-mapper` agents to regenerate `.planning/codebase/` with the updated state of the repo:

| Agent | Focus | Writes |
|-------|-------|--------|
| mapper-tech | tech | `STACK.md`, `INTEGRATIONS.md` |
| mapper-arch | arch | `ARCHITECTURE.md`, `STRUCTURE.md` |
| mapper-quality | quality | `CONVENTIONS.md`, `TESTING.md` |
| mapper-concerns | concerns | `CONCERNS.md` |

Spawn all 4 in a single message (parallel). Wait for all 4 to complete and report which documents were created/updated.

### 7c — Create PR

1. Commit any uncommitted changes on the current branch:
   ```bash
   git add -A && git commit -m "feat: complete {plan-name} build"
   ```
2. Push the branch to remote:
   ```bash
   git push -u origin plan/{plan-name}
   ```
3. Generate PR content from the plan's Summary and Acceptance Criteria sections.
4. Create a PR to `main`:
   ```bash
   gh pr create --title "feat: {plan-name}" --body "..."
   ```
   - PR title: `feat: {plan-name}` (derived from plan file name, humanized)
...
0. **Create git branch** `plan/{plan-name}` before any work starts
