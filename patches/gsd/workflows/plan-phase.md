...
@/home/edouard-gouilliard/.claude/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs init plan-phase "$PHASE")
```
...
```bash
PHASE_INFO=$(node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
```
Task(
  prompt="First, read /home/edouard-gouilliard/.claude/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
**If found:**
1. Read validation template from `/home/edouard-gouilliard/.claude/get-shit-done/templates/VALIDATION.md`
...
```bash
node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
```
Task(
  prompt="First, read /home/edouard-gouilliard/.claude/agents/gsd-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
2. Read `workflow.auto_advance` from config:
   ```bash
   AUTO_CFG=$(node /home/edouard-gouilliard/.claude/get-shit-done/bin/gsd-tools.cjs config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
Plans ready. Mapping codebase, then spawning execute-phase...
```

**Map codebase before execution:**

Spawn map-codebase and wait for completion before proceeding:
```
Task(
  prompt="
    <objective>
    Refresh the codebase map after planning Phase ${PHASE}.
    </objective>
    <execution_context>
    @/home/edouard-gouilliard/.claude/get-shit-done/workflows/map-codebase.md
    </execution_context>
    <instructions>
    Follow all steps in map-codebase.md.
    If .planning/codebase/ already exists, automatically choose Refresh (delete and remap) without asking the user.
    Do NOT use the Skill tool or /gsd: commands.
    </instructions>
  ",
  subagent_type="general-purpose",
  description="Map codebase after Phase ${PHASE} planning"
)
```
...
Task(
  prompt="
    ...
    <execution_context>
    @/home/edouard-gouilliard/.claude/get-shit-done/workflows/execute-phase.md
    @/home/edouard-gouilliard/.claude/get-shit-done/references/checkpoints.md
    @/home/edouard-gouilliard/.claude/get-shit-done/references/tdd.md
    @/home/edouard-gouilliard/.claude/get-shit-done/references/model-profile-resolution.md
    </execution_context>
...
Output the `<offer_next>` block, then automatically refresh the codebase map.
...
◆ Refreshing codebase map after planning...
```
...
- [ ] map-codebase runs automatically after planning completes (both auto and non-auto paths)
</success_criteria>
